...
#include "CubismShaderInject.h" // Familiar: runtime shader-source injection (CLT, no offline metallib)
...
void CubismShader_Metal::GenerateShaders(id<MTLDevice> device)
{
    ...
    //シェーダライブラリのロード（.metal）
    // --- Familiar patch -------------------------------------------------------
    // Prefer compiling the injected MetalShaders.metal *source* at runtime (the
    // OS runtime Metal compiler needs no Xcode), so the avatar builds under the
    // Command Line Tools. Fall back to the upstream bundle-`.metallib` path when
    // no source was injected. See CubismShaderInject.h.
    id<MTLLibrary> shaderLib = nil;
    const char* injectedShaderSrc = CubismGetMetalShaderSource();
    if (injectedShaderSrc)
    {
        NSError* compileErr = nil;
        shaderLib = [device newLibraryWithSource:[NSString stringWithUTF8String:injectedShaderSrc] options:nil error:&compileErr];
        if (!shaderLib)
        {
            NSLog(@" ERROR: Familiar runtime shader compile failed: %@", compileErr);
        }
    }
    if (!shaderLib)
    {
